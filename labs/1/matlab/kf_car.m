...
% The covariance matrix for w is Q and R for v.
% 
% For estimation we use the Kalman Filter
%
% xhat(k+1|k) = A * xhat(k|k) + B * u(k)
% P(k+1|k) = A * P * A^T + G * Q * G^T
% 
% K(k+1) = P(k+1|k) * C^T * inv(C * P(k+1|k) * C^T + D * R * D^T)
...
wStdP = 0.01; % Simulated noise on position
wStdV = 0.1;   % Simulated noise on velocity
